...
void apapalfig1()
{
  ...
  for (int iter=0; iter<3; iter++) {
    hpaps[iter] = (TH1D *) infs[iter]->Get(Form("NumOutPckstarPAPtpcM%sPsi3", cents[iter]));
    preparehist(hpaps[iter], iter);
  }

  TH1D *hramka = new TH1D("hramka",";k* (GeV/#it{c});C(k*)",100,0.0,0.5);
  preparehist(hramka, 0);
  hramka->SetMinimum(0.91);
  hramka->SetMaximum(1.26);

  TCanvas *canpapcfs = new TCanvas ("canpapcfs","canpapcfs",1200,800);
  canpapcfs->cd();
  preparepad();
